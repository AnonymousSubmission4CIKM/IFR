...
class Config(object):
    def __init__(self, args):
        ...
        self.base_model_path = os.path.join(args.base_path, 'models-ifr', args.job_name)
...
def final_test(config, model_list, test_iter):
    ret_list = []
    for model in model_list:
        ...
        with torch.no_grad():
            for texts, labels in test_iter:
                outputs = model(texts)
                ret_ = outputs.cpu().numpy().tolist()
                ret_list_tmp.extend(ret_)
                labels_all.extend(labels.cpu().detach().numpy().tolist())
        ret_list.append(ret_list_tmp)
    final_ret_ = [[0.0 for i in range(len(ret_list[0][0]))] for j in  range(len(ret_list[0]))]
    for i in range(len(ret_list)):
        for j in range(len(ret_list[i])):
            for k in range(len(ret_list[i][j])):
                final_ret_[j][k] += float(ret_list[i][j][k])
    for i in range(len(final_ret_)):
        for j in range(len(final_ret_[i])):
            final_ret_[i][j] /= float(len(ret_list))
    predict_all = np.argmax(np.array(final_ret_), axis = 1)
    acc = metrics.accuracy_score(labels_all, predict_all)
    report = metrics.classification_report(labels_all, predict_all, target_names=config.class_list, digits=4)
    confusion = metrics.confusion_matrix(labels_all, predict_all)
    logging.info('Final Test acc: {}'.format(acc))
    logging.info("Final Precision, Recall and F1-Score: ")
    logging.info(report)
    logging.info("Confusion Matrix: ")
...
def test_inaccurate_list(config, model_list, inaccurate_iter):
    ...
    for i in range(len(final_ret_)):
        for j in range(len(final_ret_[i])):
            final_ret_[i][j] /= float(len(ret_list))
    return np.asarray(labels_), np.asarray(final_ret_)
...
def random_choice(config, filtered_subset, filtered_subset_labels, seed_):
    random.seed(seed_)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='IRA')
    ...
    model_list = []
    start_model_path = os.path.join(config.base_model_path, '0_model.ckpt')
    if not os.path.exists(start_model_path):
        ...
        torch.save(model.state_dict(), start_model_path)
        acc_origin = 0
        iteration = 0
        ...
        while(iteration < config.iteration_threshold):
            ...
            _, logis = test_inaccurate_list(config, model_list, inaccurate_iter)
            ...
            for i in range(int(config.choice_num)):
                ...
                filtered_idx, chose_data, chose_labels = random_choice(config, filtered_subset, filtered_subset_labels, i * 10 + iteration)
            ...
            torch.save(model.state_dict(), os.path.join(config.base_model_path, str(iteration) + '_model.ckpt'))
            model_list.append(copy.deepcopy(model))
            # if len(model_list) > 5:
            #     model_list = model_list[-5:]
            print('Test acc: {0: >6.4%}'.format(acc_t))
    else:
        for i in range(config.iteration_threshold):
            tmp_model_path = os.path.join(config.base_model_path, str(i) + '_model.ckpt')
    ...
    final_test(config, model_list, test_iter)
    print('Log saved at: {}'.format(config.log_path))
